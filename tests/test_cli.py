...
@pytest.fixture
def mock_auto_pvs1_predict_success():
    """Fixture to mock AutoPVS1 predict method with a success response."""
    with patch("src.autoPVS1.AutoPVS1.predict") as mock_predict:
        mock_predict.return_value = "Pathogenic"
        yield mock_predict


@pytest.fixture
def mock_auto_pvs1_predict_failure():
...
def test_classify_command_success(mock_auto_pvs1_predict_success):
    """Test the 'classify' command with a mocked success response from AutoPVS1.predict."""
    result = runner.invoke(app, ["NM_000038.3:c.797G>A", "--genome-release", "GRCh38"])
    assert result.exit_code == 0
    assert "" in result.stdout


def test_classify_command_failure(mock_auto_pvs1_predict_failure):
    """Test the 'classify' command with a mocked failure response from AutoPVS1.predict."""
    result = runner.invoke(app, ["NM_000038.3:c.797G>A", "--genome-release", "GRCh38"])
    assert result.exit_code == 0
